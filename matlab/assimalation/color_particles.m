        ...
        if env(x,y,1) == 1 %if the agent is a robot
            ...
            for i = x-5:x+5
                for j = y-5:y+5
                    ...
                    %probability
                    if env(i,j,1) == 1
                        for trade = 1:floor(.1*env(i,j,5))
                            %calculate probability weights
                            W = [env(i,j,2),env(i,j,3), env(i,j,4)]./ env(i,j,5);
                    ...
                    if sqrt((x - center)^2 + (y-center)^2) < 7.1 %within 5 squares
                        ...
                        elseif theta < 0 %green
                            env(x,y,3) = env(x,y,3) + 5;
                        elseif theta < 120 %blue
                            env(x,y,4) = env(x,y,4) + 5;
                        else
                            env(x,y,2) = env(x,y,2) + 5;
                        end
