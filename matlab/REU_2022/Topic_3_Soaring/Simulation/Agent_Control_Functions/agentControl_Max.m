...
%         assert(localAgents(i).isAlive,"Neighborhood function bronk")
    ...
    separationZFactor = SL.neighborRadius / (SL.separationHeightWidth / 2);
    relPosXYZSep = relPosXYZ;
    relPosXYZSep(:,3) = separationZFactor*relPosXYZSep(:,3);
    distXYZSep = vecnorm(relPosXYZSep,2,2);
    ndistXYZSep = distXYZSep/SL.neighborRadius;
    ...
    %% Cohesion
    ascendCaringFactor = SL.cohesionAscensionMult*(1 - (currentPosXYZ(3)/SL.agentCeiling))^0; % 0 to whatever the max is
    ...
    weightCohesion = max(theirRelVelZ/SL.cohesionAscensionMax,0.1).^2*ascendCaringFactor;
    % Set weights of sinking/weakly thermalling agents to 1
%     weightCohesion(theirRelVelZ <= SL.cohesionAscensionIgnore) = 1;
    weightCohesion(relPosXYZ(:,3) <= SL.cohesionHeightIgnore) = 0;
    currentAgent.neighborData.cWeight = weightCohesion;
    cohesionVecs = weightCohesion.*ndiffXY.*ndistXY.^1;
    cohesionVEC  = sum(cohesionVecs,1) / numNeighbors;
    
    %% Separation and Alignment
%     seenSA = abs(relPosXYZ(:,3)) < SL.separationHeightWidth/2;
    seenSA = abs(relPosXYZ(:,3)) < SL.neighborRadius;
    separationVecs(seenSA,:) = - ndiffXY(seenSA,:)./ndistXYZSep(seenSA,:).^2;
    separationVecs(~seenSA,:) = [];
    separationVEC = sum(separationVecs,1);

    alignmentVecs(seenSA,:) = (theirVelXY(seenSA,:)-currentVelXY)./ndistXY(seenSA,:).^1;
