...
for step = 1:steps
    ...
    renderBoids(currentPositions,numAgents,Ceiling,agentScale);
    hold on
    ...
    for agent=1:numAgents
        if agentPositions(step,agent,4) > 0 % agent must not be dead
            ...
            [newAccel, newVel, newPos] = stepSim(agentPositions, agentVelocities, step, agent, dt, numAgents, wallMag, simParams, sinkRate);
...
function [tempAccel, newVel, newPos] = stepSim(positions, velocities, step, agent, dt, numAgents, wallMag, simParams, sinkRate)
...
function renderBoids(positions,numAgents,Ceiling,agentScale)
