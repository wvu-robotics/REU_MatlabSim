function agentControl_Baseline(currentAgent, localAgents, thermalStrength, target, SL)
%% Define Items
...
if (numLocalAgents > 0)
    ...
    currentAgent.neighborData.cWeight = 1;
    cohesionVecs = ndiffXY.*ndistXY.^SL.cohPower;
    vector_cohesion  = sum(cohesionVecs,1) / numLocalAgents;
    
    %% Separation and Alignment
    separationVecs = -ndiffXY.*ndistXYZ.^SL.sepPower;
    vector_separation = sum(separationVecs,1) / numLocalAgents;

    alignmentVecs = theirRelVelXY.*ndistXY.^SL.aliPower;
