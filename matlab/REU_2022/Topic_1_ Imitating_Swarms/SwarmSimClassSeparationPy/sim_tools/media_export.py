...
def export(export_type, name, agentPositions, agentVels, params=sim.SimParams(), controllers=[], vision_mode=False, progress_bar=False, framecap=20):
    ...
    if export_type == ExportType.INTERACTIVE:
        ...
        toGIF(name, agentPositions, params, colors,
              vision_mode=vision_mode, progress_bar=progress_bar)
    elif export_type == ExportType.MP4:
        toMP4(name, agentPositions, params, colors, vision_mode=vision_mode,
              progress_bar=progress_bar, framecap=framecap)
...
def exportTrajectories(name: str, agentPositions, params: sim.SimParams, startTime: int, duration: int, size=800):
    ...
    fig = px.line(df, x="x", y="y", color="agent",
                  labels={
                      "x": "X Position (m)",
                      "y": "Y Position (m)"
                  }
    ...
    fig.update_layout(yaxis_range=[-params.enclosure_size, params.enclosure_size],
                      xaxis_range=[-params.enclosure_size, params.enclosure_size])
...
def toPandasFrame(agentPositions, colors=[], params=sim.SimParams()):
    ...
    if colors == []:
        ...
        # should already be num_agents long
        colors = np.tile(colors, (steps+1))
    return pd.DataFrame(
        {
...
def plot(i, df, params=sim.SimParams(), vision_mode=False, write=False):
    ...
    if write:
        fig.write_image("frames/fig"+str(i).zfill(5)+".png")
    return fig
...
def toGIF(name, agentPositions, params=sim.SimParams(), colors=[], vision_mode=False, progress_bar=False):
    ...
    if (1/params.dt) < maxGifRate:
        for i in (tqdm(range(0, len(agentPositions))) if progress_bar else range(0, len(agentPositions))):
            ...
            # getting rid of file writes,slightly faster
            fig_bytes = fig.to_image(format='png')
            buf = io.BytesIO(fig_bytes)
            img = Image.open(buf)
            im = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)
        ...
        for i in (tqdm(range(0, len(agentPositions))) if progress_bar else range(0, len(agentPositions))):
            ...
            if (i % n_sample_frame == 0):
                ...
                im = cv2.cvtColor(np.array(img), cv2.COLOR_RGB2BGR)
                frames.append(img)
    # implement fps capping if this is good
    imageio.mimsave(name+'.gif', frames, fps=min(1/params.dt, maxGifRate))
...
def toMP4(name, agentPositions, params=sim.SimParams(), colors=[], vision_mode=False, progress_bar=False, framecap=60, threads=8):
    ...
    video = cv2.VideoWriter(name+'.mp4', cv2.VideoWriter_fourcc(*'mp4v'),
                            min(1/params.dt, framecap), (500, 500), True)

    n_sample_frame = 1
    if (1/params.dt) < framecap:
        n_sample_frame = int(1/(params.dt*framecap))
    ...
    ims = pool.map(partial(plotToBytes, df=df, params=params, vision_mode=vision_mode,
                   write=False), range(0, len(agentPositions), n_sample_frame))
